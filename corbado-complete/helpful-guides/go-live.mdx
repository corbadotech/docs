...
If you use one of our Backend SDKs, you need to update the [Frontend API](/corbado-complete/overview/corbado-api-structure/frontend-api) configuration and set it to the CNAME you just set up. This enables the Backend SDKs to verify the JWT issuer (the `session-token` is a JWT) which will be set to the CNAME.
