...
<Note>Previously, the session token was referred to as a short-term session, which is now **deprecated**. However, it is still stored in a cookie named `cbo_short_session` for backward compatibility.</Note>
