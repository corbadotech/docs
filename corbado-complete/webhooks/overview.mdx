...
- Webhooks should not be relied upon for mission-critical functionality since delivery is not guaranteed

## Corbado webhooks

Corbado webhooks allow you to subscribe to specific events that occur in Corbado, such as when a user is created or deleted. When an event occurs, Corbado sends an HTTP `POST` request to the webhook's configured URL. The request includes a JSON payload with information about the event.
