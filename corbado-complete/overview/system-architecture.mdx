...
The Frontend API serves as the foundation for our UI Components. While it's possible to interact with the Frontend API directly, we don't recommend this approach due to the complexity of authentication processes like signup and login.

The Frontend API provides both authenticated and unauthenticated endpoints. Authenticated endpoints require a valid Corbado Session, which is issued to users upon successful authentication.
...
The Backend API provides the core functionality of Corbado's Authentication system and is utilized by both the Frontend API and your backend application. It enables you to perform essential operations such as managing users and their associated identifiers.

The Backend API implements HTTP Basic Authentication and is restricted to clients capable of securely storing secrets. For authentication, use your project ID as the username and your API secret as the password. You can generate an API secret in the [developer panel](https://app.corbado.com/settings/credentials).
