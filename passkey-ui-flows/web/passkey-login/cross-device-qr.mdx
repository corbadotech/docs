...
This approach uses the operating system's built-in **hybrid transport** functionality (defined in CTAP 2.2) for secure cross-device authentication.
...
<Steps>
    ...
    <Step title="OS displays QR code screen">
        ...
        - Once a phone has been linked to this computer, the OS can offer it directly on later sign-ins and the QR step is skipped. Bluetooth is still required.
    </Step>
    <Step title="User scans QR and authenticates">
        - User scans the QR code using their mobile device where the passkey is stored.
        - **Bluetooth proximity check:** The OS verifies both devices are physically close (anti-relay protection).
