...
<Info>
    **Technical Implementation:** This flow uses `preferImmediatelyAvailableCredentials` — an `ASAuthorizationController` request option on iOS 16+ and a `GetCredentialRequest` parameter on Android — to ensure the overlay only displays when passkeys are immediately available on the device. On the web, Chrome 149+ offers the equivalent through immediate mode (`uiMode: 'immediate'`). Because it excludes credentials that are not stored locally, it also rules out security keys and cross-device authentication (CDA); if you need those, a different configuration must be used - contact us for implementation guidance.
</Info>
