...
The app cannot check which passkeys are available from the current device or credential provider before starting the request. A server record proves that the account has a passkey, but not that the passkey is available here.
