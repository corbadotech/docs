...
description: "A practical framework for comparing passkey implementations and adoption potential."
---

This benchmark turns the [flow catalogue](/passkey-ui-flows/overview#flow-catalogue) into product-neutral requirements. Each flow includes acceptance criteria that can be used for implementation reviews, tickets and provider comparisons.
...
Rate web and native separately, then use the lowest result across the included applications. Always show the individual results and state what was not included. For example: **Incomplete overall; web iOS: High; native: Not reviewed**.
