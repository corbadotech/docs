...
To verify if a user is logged in your {language} application needs to verify the short-term session (represented as JWT). Corbado utilizes the **RS256** algorithm (see [JWT security](/corbado-complete/sessions/security/jwt-security) for more details) for its JWTs.
