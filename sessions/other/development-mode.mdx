...
When developing locally the host (e.g. on `http://localhost`) differs from the one of the Frontend API (e.g. `https://<project ID>.frontendapi.corbado.io`). The browser sees this as third-party and does not send any cookies (e.g. in [session refresh](/sessions/other/refresh) process).
