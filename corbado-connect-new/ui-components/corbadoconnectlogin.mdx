...
<head>
  ...
  <script type="module">
    ...
    Corbado.mountCorbadoConnectLogin(passkeyLoginElement, {
      ...
      // Called when passkey login is not possible or fails critically
      onFallback: (identifier, errorMessage) => {
        console.log(`Fallback triggered for '${identifier}' with message: ${errorMessage}`);
        // Show your fallback UI (e.g., a password field).
        // The identifier might be pre-filled if the user provided one.
      },
    });
  </script>
</head>
