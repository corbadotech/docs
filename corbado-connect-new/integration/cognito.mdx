...
description: "Integrate Corbado Connect with Amazon Cognito to enable passkey-based, passwordless authentication. "
...
The flow is illustrated in detail [here](/corbado-connect-new/flows/user-sign-up).
...
Before rendering the component, we need a short-lived `connectToken` (see [here](/corbado-connect-new/ui-components/overview#3-connecttokens-ctk) for more details) from Corbado's Backend API. This token authorizes the creation of a passkey for a specific, authenticated user.
...
<Info>
For a detailed explanation of all available props for this component, please see the [CorbadoConnectAppend component documentation](/corbado-connect-new/ui-components/corbadoconnectappend).
</Info>
...
The flow is illustrated in detail [here](/corbado-connect-new/flows/user-login).
...
sequenceDiagram
    ...
    participant Client
    participant Cognito
    participant Lambda as verify_auth_challenge_response
    box "Corbado Backend"
        participant BAPI
        participant Service
        participant DB
    ...
    Client->>+Cognito: InitiateAuth (CUSTOM_AUTH)
    Note over Cognito: Runs define_auth then create_auth lambdas (DefineAuthChallenge → CreateAuthChallenge).<br/>cognitoID stored in private parameters.
    Cognito-->>-Client: Return CUSTOM_CHALLENGE

    Note over Client: Performs WebAuthn ceremony with FAPI<br/>to get signedPasskeyData token.

    Client->>+Cognito: RespondToAuthChallenge<br/>(Answer: signedPasskeyData)
    Cognito->>+Lambda: Invoke with token and private cognitoID
    Lambda->>+BAPI: POST /passkey/verifySignedData<br/>(signedPasskeyData, cognitoID)

    BAPI->>+Service: Call internal service to verify data
    Service->>Service: Validate token cryptographic signature & claims (exp)
    
    Service->>+DB: Start Database Transaction
    Note over Service: Extracts challengeId/jti and user 'sub' from token
    Service->>DB: Find challenge by ID with status 'completed'
    DB->>Service: Return challenge session from DB
    Note over Service: If session is invalid or not found, reject
    Service->>DB: Update challenge status to 'consumed'
    DB-->>-Service: Commit Database Transaction
    
    Note over Service: Match cognitoID from Lambda with user 'sub' from token
    Service-->>-BAPI: Return success
    BAPI-->>-Lambda: Return verification result
    Lambda-->>-Cognito: Return answerCorrect: true
    Note over Cognito: Final define_auth run sees success (challengeResult: true)
...
<Info>
For a detailed explanation of all available props for this component, please see the [CorbadoConnectLogin component documentation](/corbado-connect-new/ui-components/corbadoconnectlogin).
</Info>
