...
Your backend receives an association token in the [Corbado Backend API's response](https://apireference.cloud.corbado.io/backendapi/#tag/Association-Tokens/operation/AssociationTokenCreate). You need the association token for the next step to consume it for a passkey.
