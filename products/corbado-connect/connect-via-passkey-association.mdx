---
title: "🔗 Connect via Passkey Association"
...
For customers who already have an existing application and user base, Corbado provides a convenient way to offer these users passkeys as well - **without migrating and changing the entire existing authentication logic.** Via web components (`<corbado-passkey-associate/>` and `<corbado-passkey-associate-login/>`), users can add a passkey after they were authenticated with the existing, current authentication method.
...
![Create Association Token](/images/products/corbado-connect/connect-via-passkey-association/create-association-token.png)

To start the passkey association process, you need to create an association token for a user that you have previously authenticated with your existing authentication system. This token can be created from your backend by calling the [Corbado Backend API](../../backend-integration/corbado-api-structure/backend-api). You need to send a POST request to [AssociationTokenCreate](https://api.corbado.com/docs/api/#tag/Association-Tokens/operation/AssociationTokenCreate) endpoint.

Your backend receives an association token in the [Corbado Backend API's response](https://api.corbado.com/docs/api/#tag/Association-Tokens/operation/AssociationTokenCreate). You need the association token for the next step to consume it for a passkey.
...
![Create Passkey with Association Token](/images/products/corbado-connect/connect-via-passkey-association/create-passkey-with-association-token.png)
...
![Create Passkey with Association Token](/images/products/corbado-connect/connect-via-passkey-association/login-with-passkey.png)
