...
-   **Progress Preservation:** If a user refreshes the page during signup, their progress is maintained. The component automatically resumes the authentication flow at the correct step.
-   **Flexible Back Navigation:** Users can easily navigate back to previous steps in the signup process to review or modify information without having to restart from the beginning.
-   **Direct Screen Access:** Users can directly access the signup or login screens using `#signup-init` (or `#register`) and `#login-init` (or `#login`) in the URL.
...
<Note>
    ...
    [Corbado Provider](/frontend-integration/ui-components/corbadoprovider).{" "}
</Note>
