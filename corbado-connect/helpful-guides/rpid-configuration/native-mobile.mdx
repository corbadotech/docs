...
- For **iOS** applications, this involves configuring an **Apple App Site Association (AASA)** file
...
- Write little introduction
- Explain that for iPhone testing there need to be done some configs and link to iPhone testing guide (needs to be written)

### 2.1. Apple App Site Association (AASA) file

- Explain AASA file
- Show link to example file with addon that this is explained in detailed in the hosting section below (link to this section)
- Show diagram from Flutter integration guide (remove Flutter/adapt to iOS, make production example with RPID example.com, name Runner.entitlements Example.entitlements)
- Explain caching with ?mode=developer
- Explain Example.entitlements file and the entry is basically the RPID and that the iOS application will use this entry, compare it to the RPID and load the AASA file from there

### 2.2. Application Identifier Prefix and Bundle Identifier

- Explain Application Identifier Prefix and Bundle Identifier

### 2.3. Hosting of Apple App Site Association (AASA) file
