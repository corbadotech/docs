...
> **Important:** Only authenticated users are allowed to create new passkeys. You must generate a `connectToken` (see [here](/corbado-connect/concepts/connect-token)) on your backend to validate that the user session is legitimate. This token is then passed to the component.
