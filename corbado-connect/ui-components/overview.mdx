...
1. **Server generates a CTK**
   Your server (e.g., "Customer Backend") makes a secure API call to Corbado's `POST /v2/connectTokens` endpoint (see [API Reference](/api-reference/backend-api/connecttokens/post-connecttokens)). This endpoint identifies the user (for example, through a `CustomerIdentifier`) and specifies the intended action (`"passkey-append"`, `"passkey-list"`, `"passkey-delete"`, etc.).
