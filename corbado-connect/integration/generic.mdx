...
The key to user login integration is the `signedPasskeyData` - a short-lived, single-use JSON Web Token (JWT) that proves a successful passkey authentication with Corbado. This token serves as the bridge between Corbado's passkey verification and your authentication system.

There are multiple ways to integrate the passkey login flow with your authentication system:

1. **System Extension**: If your authentication system supports plugins or custom authentication methods, you can extend it to handle passkey verification and create a session.
2. **Custom Endpoint**: Implement a new endpoint in your backend that verifies the passkey authentication and creates a session.
3. **Corbado Action**: Use Corbado's action system to handle the authentication flow directly.

We will explain each option in the next sections.
...
sequenceDiagram
    ...
    IdP-->>-Frontend: Return session tokens
```
...
sequenceDiagram
    ...
    IdP-->>-Backend: Return session tokens
...
app.post('/auth/passkey/verify', async (req, res) => {
    try {
        ...
        // 4. Return session information to the client
        res.json({
...
sequenceDiagram
    ...
    IdP-->>-Actions: Return session tokens
    Actions-->>-FAPI: Return session tokens
    FAPI-->>-Frontend: Return session tokens
```
...
- You prefer to handle authentication logic in Corbado's environment
- You need a quick integration path
- Your authentication system has a simple session creation API

<Info>
The action environment provides secure access to your authentication system's API. Make sure to configure the necessary credentials and permissions in your Corbado project settings.
</Info>
