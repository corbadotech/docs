...
graph TD
    ...
    B -- Yes --> H["S2: Attempt Passkey Login<br/>(CorbadoConnectLogin)"];
    H -- Success --> AuthSession["Authenticated MFA Session"];
    H -- Not available / Fallback --> D;
    D -- Success --> AuthSession;
    AuthSession --> F["S1: Passkey Enrollment<br/>(CorbadoConnectAppend)"];
    F -- "Complete / Skip" --> Dashboard["Dashboard / Start Page"];
    Dashboard -- "Navigate to Settings" --> I["S3: Passkey Management<br/>(CorbadoConnectPasskeyList)"];
