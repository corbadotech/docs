...
{/*
TODOS:
- Copy passkey management from Kostas
- Add "How it Works" sections (Kostas user journey)
- Rename steps
- Implement session handling with session ID and cookie storage
*/}
...
graph TD
    ...
    E --> F["S1: Passkey Enrollment<br/>(CorbadoConnectAppend)"];
    F --> G{Passkey Available};
    G -- Subsequent Visits --> H["S2: Passkey Login<br/>(CorbadoConnectLogin)"];
    H --> E;
    G -- Manage --> I["S3: Passkey Management<br/>(CorbadoConnectPasskeyList)"];
    I --> G;
```
...
app.post('/auth/passkey/verify', async (req, res) => {
    try {
        ...
        if (data.verificationResult !== "success") {
            return res.status(400).json({ error: 'Invalid signedPasskeyData' });
        }
...
onComplete: async (result, clientState) => {
    ...
    // 1. Store session ID in a cookie (this is vulnerable to XSS 
    // attacks which is a disadvantage of this approach compared 
    // to option 2 for example)
