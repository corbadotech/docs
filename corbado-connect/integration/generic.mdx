...
The cornerstone of user login integration is the `signedPasskeyData` — a short-lived, single-use JSON Web Token (JWT) that confirms successful passkey authentication with Corbado. This token acts as the link between Corbado's passkey authentication and your authentication system, which ultimately needs to establish a session.
...
In each option, you must create a session in your authentication system at the end. For simplicity and documentation purposes, we use the term "session token" here. However, you have complete flexibility and can use opaque session IDs or JWTs, for example. The storage of the session identifier, whether as a cookie, in local storage, or in memory, is beyond the scope of this documentation.
