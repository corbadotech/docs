...
As we have integrated with the existing authentication system, session creation and storage are managed by it. Therefore, after a successful passkey login, we only need to redirect:
...
When using the custom endpoint approach, the `onComplete` callback needs to send the `signedPasskeyData` to your verification endpoint and handle the session creation:
...
onComplete: async (signedPasskeyData, clientState) => {
    try {
        ...
        // 2. Handle the session based on your chosen storage method
        if (result.sessionId) {
            // Storage method A: Store session ID in an HTTP-only cookie (recommended)
            // This is typically set by the backend in the response headers
            
            // Storage method B: Store session ID in localStorage
            localStorage.setItem('sessionId', result.sessionId);
            
            // Storage method C: Store session ID in memory
