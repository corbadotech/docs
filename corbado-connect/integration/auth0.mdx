...
- **No PII**: **Corbado Connect** does **not** store any personal information (PII) such as names, emails, or phone numbers.
...
`signedPasskeyData` comes from a cookie and is passed to your backend. This data acts like an authentication token - without valid `signedPasskeyData`, the login process cannot proceed.