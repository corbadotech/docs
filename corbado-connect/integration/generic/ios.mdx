...
With [Conditional UI](https://www.corbado.com/glossary/conditional-ui), a user can login with an existing passkey without providing her login identifer (e.g. email). To handle this case you need to call the `loginWithoutIdentifier()` method from the **Corbado iOS SDK**.

If the login has been successful (`.done`), we need to verify the `signedPasskeyData` with the backend using the helper method `verifySignedPasskeyData()` we implemented above.
...
let nextStep = await corbado.loginWithoutIdentifier(cuiChallenge: cuiChallenge, conditionalUI: false) {
    // Handle loading state for example
}
...
case .done(let signedPasskeyData, let username):
    do {
        ...
        if success {
            // Login has been successful, create session using sessionId
        } else {
            // Login has not been successful, show error message
        }
    } catch (let error) {
        // Handle error
    }
...
case .initSilentFallback(let email, _):
    // Fallback is set, show own login screen
...
case .error(let error, let triggerFallback, _, let username):
    ...
    case .PasskeyDeletedOnServer(let message):
        // Passkey has been deleted on server-side but still exists
        // on client-side (otherwise we would not have got here). Show
        // message e.g. "Passkey was deleted on server".
    default:
...
With textfield login, the user provides her login identifier (e.g. email). To handle this case you need to call the `loginWithIdentifier()` method from the **Corbado iOS SDK**.

Again, if the login has been successful (`.done`), we need to verify the `signedPasskeyData` with the backend using the helper method `verifySignedPasskeyData()` we implemented above.

```swift
// Take email from .initTextField(), see above
...
case .error(let error, let triggerFallback, _, let username):
    ...
    switch error {
    case .UserNotFound:
        // Show error message e.g. "There is no account registered to that email address."
    default:
        // Show generic error message
    }
...
[OneTap](/corbado-connect/features/one-tap-login) login is an additional feature of **Corbado Connect** where the login identifier (e.g. email) has been stored client-side so that the user does not have to provide it. To handle this case you need to call the `loginWithOneTap()` method from the **Corbado iOS SDK**.

After this it is exactly like textfield login, you need to verify `signedPasskeyData`.

```swift
let nextStep = await corbado.loginWithOneTap()

// Handle nextStep as in textfield login (see above)
```
...
Fallback login means that passkey login did not work out (for multiple reasons explained above) and you need to show your own login screen. This setup adds a lot of robustnest to the overall solution: no matter what if there is somethign wrong you show your own login screen and your users login in using traditional methods like email and password.
