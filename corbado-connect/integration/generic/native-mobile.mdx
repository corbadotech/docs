...
class AppBackend {
    ...
    /// Complete the login process by verifying signedPasskeyData with backend and obtaining an access token
    ...
    static func completeLogin(signedPasskeyData: String) async throws -> (Bool, String) {
        // 1. Set up request
        let urlString = "https://<your-backend>/auth/completeLogin"
        guard let url = URL(string: urlString) else {
            throw URLError(.badURL)
        }
        ...
        let requestPayload = CompleteLoginRequest(
        ...
        // 4. Decode the JSON response
        do {
            let decodedResponse = try JSONDecoder().decode(CompleteLoginResponse.self, from: data)
...
object AppBackend {
    ...
    // If you are using an external Identity Provider (IdP) such as Amazon Cognito or Firebase,
    // you can delegate the verification process to those systems by making a call to them.
    suspend fun completeLogin(signedPasskeyData: String): Result<Pair<Boolean, String>> {
        return try {
            // 1. Set up request
            val request = CompleteLoginRequest(signedPasskeyData)
            val jsonRequest = Json.encodeToString(CompleteLoginRequest.serializer(), request)

            // 2. Perform request
            val urlString = "https://<your-backend>/auth/completeLogin"
            val url = URL(urlString)
            val connection = url.openConnection() as HttpURLConnection
            connection.requestMethod = "POST"
            connection.setRequestProperty("Content-Type", "application/json")
            ...
            // 4. Decode the JSON response
            val jsonResponse = Json.decodeFromString(CompleteLoginResponse.serializer(), response)
...
let nextStep = await corbado.isAppendAllowed { _ in
    // Provide the access token of the currently logged-in user.
...
Now that users can append passkeys to their accounts, we can enable a truly passwordless login experience. The goal is to authenticate a user with their passkey using **Corbado Connect** and, upon successful verification, obtain a new **access token** from your system.
...
sequenceDiagram
    ...
    App->>+Backend: POST /auth/completeLogin<br/>(signedPasskeyData)
    Backend->>+BAPI: POST /v2/passkeys/verifySignedData
    BAPI-->>-Backend: Return verification result
    Backend->>+IdP: Request access token
    IdP-->>-Backend: Return access token
    Backend-->>-App: Return access token
```
...
If the login has been successful (`.done` / `.Done`), you must verify the returned [signedPasskeyData](/corbado-connect/concepts/signed-passkey-data) with your backend using the helper method `completeLogin()` implemented earlier.
...
case .done(let signedPasskeyData, let username):
    ...
    do {
        let (success, accessToken) = try await AppBackend.completeLogin(signedPasskeyData: signedPasskeyData)
        if success {
            // Backend verification successful. You now have a valid access token.
...
when (result) {
    is ConnectLoginWithoutIdentifierStatus.Done -> {
        // The passkey ceremony was successful.
        val (success, accessToken) = AppBackend.completeLogin(result.session).getOrThrow()
        if (success) {
            // Backend verification successful. You now have a valid access token.
            // Store the token and navigate the user to the logged-in area of your app.
        } else {
