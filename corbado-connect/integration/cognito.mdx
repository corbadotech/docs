...
Again, we start by integrating the `CorbadoConnectLogin` component from the `@corbado/connect-react` library. The component takes care of the entire UI and logic for handling the passkey login and passing the result to our application logic to complete the login with Cognito.
...
Because **Corbado Connect** only stores the **Amazon Cognito** username and not the users email address we need to translate the user-given email address during login to the **Amazon Cognito** username. This can be achieved using Corbado [actions](/corbado-connect/architecture/actions), which enable you to extensively customize **Corbado Connect** to suit your specific requirements.

The `pre-login` action utilizes the `AdminGetUser` command from **Amazon Cognito** to search for a user by the given email address and return it to **Corbado Connect**:
