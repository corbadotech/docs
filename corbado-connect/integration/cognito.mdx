...
All these operations follow a similar pattern: they require obtaining a [connectToken](/corbado-connect/concepts/connect-token) for the **Corbado Connect** Web UI component, which then handles the user interface and underlying functionality. In the following sections, we'll focus on the **passkey deletion process as an example**, since the other operations follow a similar pattern.
