...
sequenceDiagram
    ...
    Note over ExampleFrontend: CorbadoConnectAppend component<br />renders
    ExampleFrontend->>+ExampleBackend: Requests Corbado Connect Token
    ExampleBackend->>ExampleBackend: Verifies user session
    ExampleBackend->>+BAPI: POST /connectToken
    BAPI-->>-ExampleBackend: Returns Connect Token
    ExampleBackend-->>-ExampleFrontend: Returns Connect Token

    ExampleFrontend->>+FAPI: Executes Passkey Ceremony (WebAuthn)
    ...
    Cognito-->>-Actions: Returns success
    Actions-->>-FAPI: Returns success
    FAPI-->>-ExampleFrontend: Returns success
```
...
- post-append Action kurz anreißen und Source-Code zeigen

### Obtaining the ConnectToken

TODO Deeplink to API Reference
