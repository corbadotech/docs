...
Gradual rollout is a feature that allows you to control which users are able to interact with the passkey [Corbado Web UI Components](/corbado-connect/web-ui-components). This gives you the flexibility to introduce passkeys to your users in a phased manner, for example, by first enabling it for a small percentage of users and then gradually rolling it out to a wider audience.
...
Whenever one of the [Corbado Web UI Components](/corbado-connect/web-ui-components) is about to be displayed to a user, it checks with Corbado's backend to see if the user is eligible based on the configured rules. Based on the response, the component will either render or stay hidden.
