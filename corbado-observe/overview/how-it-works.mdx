...
That gap is what **Corbado Observe** measures. For the wider argument, see [Why you need authentication observability for CIAM](https://www.corbado.com/blog/authentication-observability).

## 2. How data flows through Observe

The [integration paths](/corbado-observe/overview/integration-paths) differ only in step 1. From step 2 onwards they run through the same pipeline.

```mermaid
flowchart TD
    subgraph capture["1. Capture"]
        A["Autocapture<br />web"]
        L["Data layer + mapping<br />web"]
        N["Native SDKs<br />iOS, Android"]
        C["Custom events<br />web"]
    end
    capture --> B["2. Send"]
    B --> D["3. Classify"]
    D --> E["4. Pre-calculate"]
...
<Steps>
  <Step title="Capture">
    [Autocapture](/corbado-observe/get-started/autocapture) observes what your web frontend already does. The [data layer](/corbado-observe/get-started/data-layer) takes a few semantic events your web app pushes and maps them centrally. The [native SDKs](/corbado-observe/get-started/native) capture system evidence in your iOS and Android apps and take facts from your screens through typed hooks. [Custom events](/corbado-observe/get-started/custom-events) are events you emit yourself on web.
  </Step>
  <Step title="Send">
  ...
  <Step title="Classify">
    Raw events are mapped onto the [data model](/corbado-observe/tracking/overview): **flows** (login, sign-up, recovery), **subflows** (passkey login, password enrollment, email OTP), **decisions**, **users** and **tags**. This is closer to [process mining](https://www.corbado.com/blog/authentication-process-mining) than to a flat event log, which is what makes it possible to reconstruct a journey including loops, retries and method switches.
  </Step>
  <Step title="Pre-calculate">
    Classified data is rolled into time series bucketed hourly, daily, monthly and total. Dashboards read a prepared series instead of scanning raw events, which is what keeps them fast on large projects. Cadence is automatic and can be tuned under [**Observe → Settings → Pre-calculation**](https://app.corbado.com/observe/settings/pre-calculation).
  </Step>
...
| Depend on a specific IdP or stack | Commercial IdP, open-source library or custom in-house implementation: it makes no difference. |
...
See [Corbado Connect](/corbado-connect/overview/welcome) if you want to add passkeys to your existing identity provider.
...
Observe complements these tools and does not replace them. It is deliberately narrow: the client side of authentication, modeled properly. For a category-by-category comparison, see [Which tool reports the real authentication success rate?](https://www.corbado.com/blog/authentication-success-rate-analytics-vendors)
