...
| Platform | Autocapture | Data layer | Custom events | Notes |
| --- | --- | --- | --- | --- |
| Web apps (SSR, SPA, MPA) | Yes | Yes | Yes | Same-document route changes are observed, so a journey survives a client-side router. |
| Web frameworks (React, Vue, Svelte, Angular, Next.js, Nuxt) | Yes | Yes | Yes | No framework-specific integration required. |
| Web views inside native apps | Yes | Yes | Yes | The SDK runs in the web view. |
| Cross-origin iframes | Yes | Yes | Yes | The SDK must run in the frame that performs the ceremony. See [Iframes and RP IDs](#3-iframes-and-rp-ids). |
| iOS (Swift) | No | No | Yes | [Native SDK](/corbado-observe/get-started/native), iOS 15 or newer. It captures system evidence on its own and takes facts from your screens through typed hooks, the native form of the data layer. |
| Android (Kotlin) | No | No | Yes | [Native SDK](/corbado-observe/get-started/native), `com.corbado:observe`, minSdk 23. Same model as iOS, plus autofill evidence. |
...
- **The SDK must run in the frame that performs the ceremony.** `navigator.credentials` calls are observed in the document that makes them. The parent document sees nothing.
- **The iframe must be permitted to perform WebAuthn.** A cross-origin iframe needs the `publickey-credentials-get` and `publickey-credentials-create` permissions policy for passkeys to work at all. That is a requirement of your own implementation. Observe does not add it, but it is the first thing to check when ceremonies appear to be missing.
...
<Warning>
  WebAuthn responses are inherently identifying: they contain credential IDs, user handles, challenges, signatures and authenticator data. The capture library removes the user name and display name from creation options, the signature from assertion responses and PRF extension results from both; credential IDs, challenges and attestation material remain. Observe collects the fields it needs for authenticator and credential insight. If you must not send user handles or credential IDs to a processor, raise this before integration so the collection profile can be scoped accordingly.
</Warning>
...
| **Facts that never reach the browser** | Anything that exists only in your native app or your backend cannot be observed. Add it with [custom events](/corbado-observe/overview/integration-paths#4-where-autocapture-stops). |
...
SDK storage and cross-tab locks are scoped by project ID. The `storage` option selects storage for device identity; it does not move all SDK state into cookies:

| Option | Behavior |
| --- | --- |
| `storage: "local"` | Device identity in local storage on the current origin (default). |
| `storage: "cookie"` | Device identity in a cookie, with an optional `cookieDomain` for sharing that identity across subdomains. |
...
The inactivity window is server-controlled. There is no initialization option for it. An Observe session is a telemetry grouping, independent of your application's authenticated session. If a flow continues across page loads, its inactivity window must span that journey. On upgrade, a valid legacy bare session ID is preserved and dated when first read. It is not split immediately. Follow the [upgrade instructions](/corbado-observe/get-started/custom-events#migrating-existing-project-storage) when introducing multiple projects.

Observe stores no advertising identifiers and does no cross-site tracking. Whether your consent management platform needs to gate the SDK is your own legal assessment. If it does, initialize Observe after consent and journeys before that point are simply not recorded. With the [data layer](/corbado-observe/get-started/data-layer), the stub and the capture start with the page and no identity is transmitted until your code signals consent, so the question becomes whether recording before consent is acceptable at all.
...
<CardGroup cols={2}>
  <Card title="Integration paths" icon="route" href="/corbado-observe/overview/integration-paths">
    Autocapture, data layer, native SDKs or custom events.
  </Card>
