...
Observe does not require directly identifying attributes. Use pseudonymous user references and review the payloads your integration sends, including custom event data and Autocapture projections. WebAuthn payloads can also contain identifying values.

Do not include passwords, form field contents or your application’s session tokens in custom events or network body projections. Autocapture does not provide generic redaction of application payloads.
...
<Warning>
  In `@corbado/autocapture` 0.6.0, WebAuthn sanitization removes creation-option `user.name` and `user.displayName`, assertion `response.signature`, and `clientExtensionResults.prf.results` from credential responses. Other fields, including unknown fields and extensions, remain. Credential IDs, user handles, challenges, authenticator data and attestation material can still identify users. Encoded blobs are not decoded or redacted. Review the collection profile before upgrading; integrations pinned to older versions retain their previous behavior.
</Warning>

**You control identity.** The user reference is a value you pass. Pass a pseudonymous identifier and keep directly identifying values out of the other fields you send.
...
In `@corbado/observe` 0.13.0, SDK storage and cross-tab locks are scoped by project ID. The `storage` option selects storage for device identity; it does not move all SDK state into cookies:
