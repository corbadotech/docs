...
Native SDKs send into the same [data model](/corbado-observe/tracking/overview) as the web SDK, so web and app journeys are comparable in the same project. Keep them apart with [applications](/corbado-observe/tracking/applications).
...
| **Concepts Autocapture will not infer** | Enrollment intent, conditional-UI spec types, known-identifier state and user-vs-system actor are deliberately not guessed. Use [custom events](/corbado-observe/get-started/custom-events) where you need them. |
...
| Android | minSdk 23. |
| iOS | Contact us for the current minimum deployment target. |
