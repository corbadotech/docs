...
The **Corbado Observe SDK** supports `applicationId` in tracker initialization options (`TrackerOptions`). The SDK lowercases the value and stamps it as a tag on every flow start.

You can pass application context in two ways:

1. Set `applicationId` once during initialization.
2. Pass `applicationId` as a tag on `flowStarted()` when the channel is only known per flow.
...
<Tabs>
  <Tab title="NPM">
    ...
    ### 3.2 Set `applicationId` per flow
