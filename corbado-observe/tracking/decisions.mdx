...
Realistic logins have many navigational options. They are the adaptive, per-application part of the model and deserve the same care as the method options. Name them for reuse across decisions, for example `back`, `switch-to-signup` or `recovery`.
...
- Send `started` on presentation and whenever options change. A re-render alone is no presentation.
