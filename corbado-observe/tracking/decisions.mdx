...
An auth-method option whose subflow auto-starts on the screen still belongs in the option set. Silent [trusted-device checks](/corbado-observe/tracking/subflows#3-12-trusted-device-check-and-enrollment) are system work, not offered methods: they have no decision option and never resolve a decision.
