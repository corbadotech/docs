...
Call it as early as the reference is known, because only an identified flow explains a failed or abandoned attempt:

- **When the identifier is submitted.** The identifier names the account the attempt targets, not the person at the keyboard. Send it on submit, after client-side validation and before the host answers. A remembered identifier from a cookie or saved field is known at flow start and goes out there.
- **When the host confirms the user.** Send the `userId` with the login result. The latest observation in the flow wins, so it supersedes the earlier identifier.
- **When an enrollment flow starts.** The user is already logged in, so the reference is known at `flowStarted()`.

A value sent early goes out for every failed attempt too, including a stranger typing someone else's identifier. Send only a value you accept in that case; otherwise pseudonymize it in the client or wait for the host.
