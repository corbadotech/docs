...
```typescript
getTracker().setUser({ userId, identifier });
getTracker().flowFinished({ flowName: "signup" });
getTracker().flowAutoFinished({ flowName: "login", finishedByFlowName: "signup" });
```
...
async function startPasswordRecovery(email: string) {
  ...
  tracker().setUser({ crossEnvironmentTransactionID });
...
async function verifyResetLink(token: string, crossEnvironmentTransactionID: string) {
  ...
  tracker().setUser({ userId: result.userId, crossEnvironmentTransactionID });
}
...
async function setNewPasswordAndLogin(token: string, newPassword: string, newPasswordInput: HTMLInputElement) {
  ...
  tracker().setUser({ userId: result.userId });
  tracker().flowFinished({ flowName: "recovery" });
  tracker().flowAutoFinished({ flowName: "login", finishedByFlowName: "recovery" });
}
```
