...
A `flowAutoFinished()` without an open parent is not fatal, but the classifier reconstructs a closed flow of that name, so send it only for a parent that was started. A flow that runs after the session already exists, typically an enrollment prompted after login, is not nested. It is a sibling flow started after the login finished, with its own `flowStarted()` and `flowFinished()`.
