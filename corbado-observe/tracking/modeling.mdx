...
Events are only the transport. A backend classifier reads each session's event stream and produces the **flows**, **decisions** and **subflow attempts** that every dashboard is built on. Raw events remain visible in the console for debugging a single user, but the classified output is what carries the value.

The classifier is not visible from your integration, so this page describes the rules it applies and a method for mapping your journeys onto them. Reason about every event you emit from the classifier's perspective. How the raw series looks is secondary. The method is the same whether the events come from the [data layer mapping](/corbado-observe/get-started/data-layer) or from [custom events](/corbado-observe/get-started/custom-events) in your code; with the data layer, each pass below fills one table of the mapping instead of placing calls in components.
...
<Steps>
  <Step title="Flow boundaries">
    For every flow, find the single best signal for when it starts, when it finishes successfully and, where your product has one, when it is skipped. Do this for all flows, top-level and nested, before anything else. The result of this pass is an exact definition of what is tracked. This pass needs the most care: a `flowFinished()` without a matching open flow invalidates the whole session's classification.
  </Step>
...
Realistic logins have many navigational options. They are the adaptive, per-application part of the model and deserve the same care as the method options. Name them for reuse across decisions.
...
- **Action-bound methods** (passkey button, social button, app confirmation) start on the action. A visible button is no attempt yet. A helper created for a button nobody pressed produces an attempt without interaction: not counted for most types, counted as an incomplete attempt for passkeys.
...
flow_finished                 { flowName: signup, userId, identifier }
flow_started                  { flowName: enrollment, touchpoint: post-signup }   chained as a sibling
