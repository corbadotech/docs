...
flow_enriched                 { userId, identifier }                    setUser()
...
subflow_started               { password-enrollment, spec: password-set }
subflow_step_started          { password-enrollment, post-response }
subflow_step_finished         { password-enrollment, post-response }
flow_enriched                 { userId, identifier }                    setUser()
flow_finished                 { flowName: signup }
...
flow_auto_finished            { flowName: login, finishedByFlowName: recovery }
```
...
```text
flow_started                  { flowName: login, touchpoint: netbank-logon, tags: { known_user: true } }
flow_enriched                 { identifier }                            setUser(), the remembered client
auth_method_decision_started  { post-identifier, options: [forget-client, password-login-known-identifier,
                                switch-to-passkey, back] }
subflow_started               { password-login, spec: password-known-identifier }   password field rendered
subflow_step_started          { password-login, post-response }
subflow_step_finished         { password-login, post-response }         resolves post-identifier
subflow_started               { trusted-device-check, spec: token }     the host checked the cookie
subflow_step_started          { trusted-device-check, post-response }
subflow_step_finished         { trusted-device-check, post-response, result: not-trusted }
auth_method_decision_started  { 2fa, options: [passkey-login-known-identifier, switch-to-app, back] }
auth_method_decision_finished { 2fa, explicitDecisionValue: switch-to-app }
auth_method_decision_started  { 2fa, options: [app-confirmation, try-another-way] }
...
subflow_step_finished         { app-confirmation, post-response }       resolves 2fa
subflow_started               { trusted-device-enrollment, spec: token }
subflow_step_started          { trusted-device-enrollment, post-response }
subflow_step_finished         { trusted-device-enrollment, post-response, result: created }
flow_enriched                 { userId, identifier }                    setUser(), host-confirmed
flow_finished                 { flowName: login, tags: { method: app, browser_trusted: true } }
flow_started                  { flowName: enrollment, touchpoint: security-settings }   chained
flow_enriched                 { userId }                                setUser()
...
subflow_step_started/finished { passkey-enrollment, get-options }
subflow_step_started/finished { passkey-enrollment, ceremony, mediation: required }
subflow_step_started/finished { passkey-enrollment, post-response }     resolves enrollment-user
flow_finished                 { flowName: enrollment }
```

Three things this series shows. The two trusted-device subflows are system work: they belong to no decision and resolve no option, and the negative check is a finished step with a result, not an error. The identifier is attached up front only because the browser remembered the client. A customer who types the client number gets the identity attached after the host confirmed the login, so an abandoned attempt carries no user reference. The `2fa` checkpoint is presented twice because the customer navigated from the method choice to the app screen, and the second presentation is the one the app confirmation resolves. The enrollment is chained after the login finished, so it is a sibling with its own identity call, and a "don't ask again" would finish it with `explicitOutcome: skipped` exactly like a skip.
