...
### Code example: passkey login

The SDK provides a dedicated class for each subflow type. Each class exposes its predefined steps as properties with `.start()`, `.finished()`, and `.error()` methods.
...
<Tabs>
  ...
  <Tab title="Conditional UI (pre-identifier)">
    No identifier is known yet. The SDK binds to the identifier input field and tracks both the provide-identifier subflow and a CUI passkey login subflow together. The CUI trigger and ceremony start are **auto-detected** by the SDK when the user interacts with the browser autofill prompt.
...
Custom steps are supported via the generic step events for additional logic around enrollment.
