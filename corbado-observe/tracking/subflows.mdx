...
<Info>
  Install and set up the **Corbado Observe SDK**, including your first tracked event, in [Precision tracking](/corbado-observe/get-started/precision-tracking). This page explains the subflow concept and documents all available subflow types.
</Info>
...
Tracks passkey-based sign-in attempts where the WebAuthn ceremony is started explicitly — by the relying party or via immediate mediation (not via Conditional UI — see [Provide identifier](#1-provide-identifier) for CUI).
