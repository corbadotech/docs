...
A "Sign in with passkey" button after the identifier is known. These examples assume the tracker is initialized before the handlers run, as described in [Custom events](/corbado-observe/get-started/custom-events); `getTracker()!` expresses that prerequisite to TypeScript:
...
```typescript
import { getTracker, sanitizeRequestOptions, sanitizeAssertionResponse } from "@corbado/observe";
...
```typescript
import { getTracker, sanitizeCreationOptions, sanitizeCreationResponse } from "@corbado/observe";
...
const handleManualEnrollment = async (userId: string) => {
  ...
  try {
    ...
    const attestationOptions = sanitizeCreationOptions(options);
    if (attestationOptions !== undefined) op.getOptions.finished({ attestationOptions });
  } catch (e) {
    op.getOptions.error(e);
    return;
  }
  ...
  try {
    ...
    const attestationResponse = sanitizeCreationResponse(response);
    if (attestationResponse !== undefined) op.ceremony.finished({ attestationResponse });
  } catch (e) {
    op.ceremony.error(e);
    return;
  }
