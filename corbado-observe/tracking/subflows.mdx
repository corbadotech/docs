...
| 3 | `subflow_step_started` | `ceremony` | User clicks the button and the browser WebAuthn ceremony begins. This step marks the interaction. |
...
<Note>
  You do not need to send `subflow_trigger` events for this subflow, because interaction is handled automatically. Explicit triggers for provide-identifier are deprecated and will be removed in a future SDK version.
</Note>
...
Tracks passkey-based sign-in attempts where the WebAuthn ceremony is started explicitly, by the relying party or via immediate mediation. This is not Conditional UI: see [Provide identifier](#1-provide-identifier) for CUI.
...
<Note>
  You do not need to send `subflow_trigger` events for this subflow, because the started `ceremony` step marks the interaction. Explicit triggers for passkey-login are deprecated and will be removed in a future SDK version.
</Note>

#### Code example

<Tabs>
  <Tab title="Immediate mediation (pre-identifier)">
    The browser offers a passkey prompt immediately when the login page loads, before the user has entered an identifier and before Conditional UI starts. **No trigger is sent**: there is no user click and the prompt is owned by the browser, so the started `ceremony` step marks the interaction.

    ```typescript
    import { getTracker } from "@corbado/observe";

    const attemptImmediatePasskeyLogin = async () => {
      const passkeyOp = getTracker().passkeyLoginFullOperation();

      // Assertion options are typically embedded in the page or pre-fetched
      // before any UI is shown, so no get-options step is tracked here.
      const options = await fetchAssertionOptions();

      let response;
      try {
        passkeyOp.ceremony.start({
          explicitSpecType: "passkey-immediate",
          assertionOptions: JSON.stringify(options),
        });
        response = await startWebAuthnAuthentication(options, { mediation: "immediate" });
        passkeyOp.ceremony.finished({ assertionResponse: JSON.stringify(response) });
      } catch (e) {
        passkeyOp.ceremony.error(e);

        // Continue with the regular login: redirect the user to your login
        // component, or, if you are already on the login page, start
        // Conditional UI on the identifier field (see Provide identifier).
        return;
      }
  ...
  <Tab title="Button click (post-identifier)">
    The user's identifier is already known. A "Sign in with passkey" button is shown, and the user clicks it to start.
...
| `explicitSpecType` | Description |
| --- | --- |
| `conditional-auto-manual` | System first attempts conditional (auto-register) enrollment, then falls back to a regular prompt if it fails, then allows the user to trigger it manually. |
...
<Note>
  You do not need to send `subflow_trigger` events for this subflow, because interaction is handled automatically. Explicit triggers for password-login are deprecated and will be removed in a future SDK version.
</Note>
...
<Tabs>
  <Tab title="Identifier + password (pre-identifier)">
    Identifier and password are entered together on the same screen (no separate identifier step before).

    ```typescript
    import { getTracker, PasswordLoginOperationFull } from "@corbado/observe";
  ...
  <Tab title="Password login (post-identifier)">
    The identifier was already submitted in a previous step; only the password is entered on this screen.
    ```typescript
    import { getTracker, PasswordLoginOperationFull } from "@corbado/observe";
