...
With `@corbado/observe` 0.13.0 or later, persisted state and cross-tab locks are scoped by `projectId`. Keep a tracker for each project and send events through the intended instance:
