...
description: "Instrument your web authentication flows with explicit, hand-written Corbado Observe events."
...
<Info>
  Custom events are hand-written tracker calls. Every call site follows the taxonomy rules on its own, so the effort grows with the number of places you instrument. Integrate from source through the [Observe data layer](/corbado-observe/get-started/data-layer) unless you deliberately want tracker calls at single points in your code, and run [Autocapture](/corbado-observe/get-started/autocapture) when the frontend cannot change. On iOS and Android, use the [native SDKs](/corbado-observe/get-started/native). See [Integration paths](/corbado-observe/overview/integration-paths).
</Info>
...
<Note>
  iOS and Android have their own page: [Get started on iOS and Android](/corbado-observe/get-started/native). The native SDKs capture system evidence on their own and take facts from your screens through typed hooks.
</Note>
