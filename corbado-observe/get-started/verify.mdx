...
| **[Management console](https://app.corbado.com/observe/debugging/integration)** | Confirms events were accepted and modeled correctly. Use the events view for the raw stream and user search to reconstruct one journey end to end. **Process** on the integration page classifies the visible sessions and precalculates the time series without waiting. |
| **[Observe CLI](/corbado-observe/tools/cli)** | The same checks from the terminal: `events-feed` for arrivals, `classify` to classify now, `classification-errors` for why a session did not classify, `session` for the classified journey. |
...
<Steps>
  ...
  <Step title="Check a failure">
    Cancel a passkey dialog and confirm that it appears as an incomplete journey. Silent failures produce no backend request at all, so this is the case worth checking explicitly.
  </Step>
</Steps>
