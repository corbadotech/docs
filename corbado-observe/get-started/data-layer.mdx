...
| **Mapping** | One module that takes over the buffer, replays it, and calls the Observe SDK. It holds every Observe name, every flow boundary, every subflow step | Corbado-shaped. Compiled into your app, published as your package, or served from Corbado's CDN |
...
Delivery is a choice you make before go-live, and it does not change your emitters:

| Mode | How | Tracking corrections need |
| --- | --- | --- |
| **Compiled in** | The module lives in your codebase and installs at startup after consent | An app release |
| **Your own package** | The module is published as your npm package and versioned like any dependency | A dependency bump |
| **Corbado CDN** | A tiny loader inserts an immutable, versioned mapping bundle. Rolling back re-points the loader | Nothing on your side. Corbado can ship a correction that is live on the next page load |

<Note>
  CDN delivery is set up together with Corbado: we host the bundle, verify a new version by injecting it into your live page in a test browser and replaying recorded journeys, and only then point the loader at it. [Talk to us](mailto:support@corbado.com) when you approach go-live.
</Note>
