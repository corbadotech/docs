...
<Info>
  Autocapture is web only. On iOS and Android, use the [native SDKs](/corbado-observe/get-started/native).
</Info>
...
<Note>
  If you integrate from your own source code, use the [Observe data layer](/corbado-observe/get-started/data-layer). It works like Autocapture: one central mapping module turns the captured signals into Observe events. The difference is where the signals come from and where the mapping lives. Your app pushes the screen events, network and WebAuthn are captured in your page, and the mapping can sit in your own codebase. [Custom events](/corbado-observe/get-started/custom-events) remain available as hand-written tracker calls and take 1 to 3 days for a typical login flow.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Combine with custom events" icon="code" href="/corbado-observe/overview/integration-paths#5-combining-paths">
