...
<Note>
  This guide assumes that you've setup your account in the Corbado developer
...
<Note>
  Note that this step is **only required** if you project is in **environment
  PROD**. If you are in environment DEV you can skip this.
</Note>

Add a new authorized origin with the ngrok URL and add localhost, including protocol (http) and port in ["Settings" - "General" - "Authorized origins"](https://app.corbado.com/app/settings/general/authorized-origins):
